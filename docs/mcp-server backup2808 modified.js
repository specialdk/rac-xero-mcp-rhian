    ...
    tools: [
      ...
      },
      {
        ...
        inputSchema: {
          type: "object",
          properties: {
            tenantId: { type: "string", description: "Xero tenant ID (optional if organizationName provided)" },
            organizationName: { type: "string", description: "Organization name (e.g., 'Mining' or 'Aboriginal Corporation')" },
            reportDate: { type: "string", description: "Report date in YYYY-MM-DD format (optional, defaults to today)" },
          },
        },
      },
      {
        ...
        inputSchema: {
          ...
          properties: {
            tenantId: { type: "string", description: "Xero tenant ID (optional if organizationName provided)" },
            organizationName: { type: "string", description: "Organization name (e.g., 'Mining')" },
          },
        },
      },
      {
      ...
      // NEW ANALYTICAL TOOLS
      {
        ...
        description: "Get manual journal entries for a specific organization to identify unusual postings",
        inputSchema: {
          type: "object",
          properties: {
            tenantId: { type: "string", description: "Xero tenant ID (optional if organizationName provided)" },
            organizationName: { type: "string", description: "Organization name" },
            dateFrom: { type: "string", description: "Start date in YYYY-MM-DD format (optional, defaults to 30 days ago)" },
            dateTo: { type: "string", description: "End date in YYYY-MM-DD format (optional, defaults to today)" },
            accountName: { type: "string", description: "Filter by specific account name (optional)" }
          },
        },
      },
      {
        name: "analyze_equity_movements",
        description: "Analyze movements in equity accounts, particularly useful for investigating the Future Fund account",
        inputSchema: {
          type: "object",
          properties: {
            tenantId: { type: "string", description: "Xero tenant ID (optional if organizationName provided)" },
            organizationName: { type: "string", description: "Organization name" },
            equityAccountName: { type: "string", description: "Specific equity account to analyze (e.g., 'Future Fund')" },
            monthsBack: { type: "number", description: "Number of months to analyze (default 12)" }
          },
        },
      },
      {
        ...
        inputSchema: {
          ...
          properties: {
            tenantId: { type: "string", description: "Xero tenant ID (optional if organizationName provided)" },
            organizationName: { type: "string", description: "Organization name" },
            accountName: { type: "string", description: "Name of the account to analyze", required: true },
            dateFrom: { type: "string", description: "Start date in YYYY-MM-DD format (optional)" },
            dateTo: { type: "string", description: "End date in YYYY-MM-DD format (optional)" }
          },
        },
      },
      {
        name: "check_bank_reconciliation",
        description: "Compare bank account balances between trial balance and actual bank feeds to identify discrepancies",
        inputSchema: {
          type: "object",
          properties: {
            tenantId: { type: "string", description: "Xero tenant ID (optional if organizationName provided)" },
            organizationName: { type: "string", description: "Organization name" },
            bankAccountName: { type: "string", description: "Specific bank account to check (optional, checks all if not provided)" }
          },
        },
      },
      {
        name: "find_unbalanced_transactions",
        description: "Find transactions or journal entries that may be causing trial balance imbalances",
        inputSchema: {
          type: "object",
          properties: {
            tenantId: { type: "string", description: "Xero tenant ID (optional if organizationName provided)" },
            organizationName: { type: "string", description: "Organization name" },
            minimumAmount: { type: "number", description: "Minimum transaction amount to analyze (default 10000)" },
            dateRange: { type: "string", description: "Date range to search (e.g., '3months', '1year', 'all')" }
          },
        },
      },
      {
        name: "get_chart_of_accounts",
        description: "Get complete chart of accounts structure to identify unusual or problematic accounts",
        inputSchema: {
          type: "object",
          properties: {
            tenantId: { type: "string", description: "Xero tenant ID (optional if organizationName provided)" },
            ...
            includeArchived: { type: "boolean", description: "Include archived accounts (default false)" }
          },
        },
      },
      {
        name: "investigate_imbalance",
        description: "Comprehensive analysis tool that investigates trial balance imbalances using multiple data sources",
        inputSchema: {
          type: "object",
          properties: {
            tenantId: { type: "string", description: "Xero tenant ID (optional if organizationName provided)" },
            organizationName: { type: "string", description: "Organization name" },
            focusAccount: { type: "string", description: "Specific account to focus investigation on (optional)" },
            analysisDepth: { type: "string", description: "Level of analysis: 'basic', 'detailed', 'comprehensive' (default 'detailed')" }
          },
        },
      },
      {
        name: "compare_periods",
        description: "Compare trial balance between different time periods to identify when imbalances were introduced",
        inputSchema: {
          type: "object",
          properties: {
            tenantId: { type: "string", description: "Xero tenant ID (optional if organizationName provided)" },
            organizationName: { type: "string", description: "Organization name" },
            fromDate: { type: "string", description: "Earlier date for comparison (YYYY-MM-DD)" },
            toDate: { type: "string", description: "Later date for comparison (YYYY-MM-DD, optional - defaults to today)" },
            accountFilter: { type: "string", description: "Filter by specific account type or name (optional)" }
          },
        },
      }
    ],
  ...
  try {
    ...
    if (name === "get_organizations") {
      ...
      if (connections.length === 0) {
        return {
          content: [{ type: "text", text: "No Xero organizations found. Please connect organizations through the Railway web dashboard first." }],
        };
      }
    ...
    if (name === "get_trial_balance") {
      ...
      if (!actualTenantId) {
        return { content: [{ type: "text", text: "❌ Error: Must provide either tenantId or organizationName" }] };
      }
    ...
    if (name === "get_journal_entries") {
      const { tenantId, organizationName, dateFrom, dateTo, accountName } = args;
      ...
      return {
        content: [{
          type: "text",
          text: `📝 JOURNAL ENTRIES ANALYSIS\n\n` +
                `Organization: ${organizationName || actualTenantId}\n` +
                `Date Range: ${dateFrom || 'Last 30 days'} to ${dateTo || 'Today'}\n\n` +
                `❌ ${mockResult.error}\n\n` +
    ...
    if (name === "analyze_equity_movements") {
      const { tenantId, organizationName, equityAccountName, monthsBack } = args;
      ...
      return {
        content: [{
          type: "text", 
          text: `📈 EQUITY MOVEMENTS ANALYSIS\n\n` +
                `Organization: ${organizationName || actualTenantId}\n` +
    ...
    if (name === "get_account_history") {
      const { tenantId, organizationName, accountName, dateFrom, dateTo } = args;
      ...
      return {
        content: [{
          ...
          text: `📚 ACCOUNT HISTORY ANALYSIS\n\n` +
                ...
                `Implementation: /api/account-history/${actualTenantId}/${encodeURIComponent(accountName)}\n` +
    ...
    if (name === "check_bank_reconciliation") {
      ...
      const trialBalance = await callRailwayAPI(`/api/trial-balance/${actualTenantId}`);
      const cashPosition = await callRailwayAPI(`/api/cash-position/${actualTenantId}`);
      ...
      const tbCashAccounts = trialBalance.trialBalance.assets.filter(acc => 
    ...
    if (name === "find_unbalanced_transactions") {
      ...
      return {
        content: [{
          type: "text",
          text: `🔍 UNBALANCED TRANSACTIONS ANALYSIS\n\n` +
                `Organization: ${organizationName || actualTenantId}\n` +
                `Minimum Amount: $${(minimumAmount || 10000).toLocaleString()}\n` +
    ...
    if (name === "get_chart_of_accounts") {
      ...
      return {
        content: [{
          type: "text", 
          text: `📋 CHART OF ACCOUNTS ANALYSIS\n\n` +
                `Organization: ${organizationName || actualTenantId}\n` +
    ...
    if (name === "investigate_imbalance") {
      ...
      result += `• Status: ${balanceCheck.debitsEqualCredits ? 'BALANCED' : 'OUT OF BALANCE'}\n`;
      
      if (!balanceCheck.debitsEqualCredits) {
        result += `• Difference: $${balanceCheck.difference.toLocaleString()}\n`;
        result += `• Severity: ${Math.abs(balanceCheck.difference) > 1000000 ? 'CRITICAL' : Math.abs(balanceCheck.difference) > 10000 ? 'HIGH' : 'LOW'}\n\n`;
        ...
        if (futureFund) {
          ...
          result += `• This single account represents ${((futureFund.balance / Math.abs(balanceCheck.difference)) * 100).toFixed(1)}% of the imbalance\n\n`;
    ...
    if (name === "compare_periods") {
      const { tenantId, organizationName, fromDate, toDate, accountFilter } = args;
